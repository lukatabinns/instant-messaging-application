...
class User {
	...
	*/
	...
	function showName($userid = 0) {
		...
		if(!$userid)
			if($this->data->banned)
				return "<strike>".$this->filter->fullname."</strike>";
			else	
				return "<font color='".$this->group->color."'>".$this->filter->fullname."</font>";

		$u = $this->db->getRow("SELECT `fullname`,`banned` FROM `".MLS_PREFIX."users` WHERE `userid` = ?i", $userid);
		$group = $this->getGroup($userid);
	
		if($u->banned)
			return "<strike>".htmlentities($u->fullname, ENT_QUOTES)."</strike>";		
		else	
			return "<font color='".$group->color."'>".htmlentities($u->fullname, ENT_QUOTES)."</font>";		
	}
