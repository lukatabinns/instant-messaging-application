...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
    <head>
		...
		<link rel="stylesheet" href="<?php echo $set->url; ?>/css/connect-list.css">
		<link rel="stylesheet" href="<?php echo $set->url; ?>/css/user-thumb.css">
...
function submitChat(){
	...
	$.ajaxSetup({cache:false});
		...
		//$('html,body').animate({scrollTop: $chatlog.offset().top + $chatlog.height()});
	...
	$(document).ready(function() {
		var timer = null;
		$('#notify').hide(); // Initially hide notification
		$('#stat').show();
		$('#msg').keypress(function() { // When key pressed
			$('#stat').hide();
			$('#notify').html('typing....').fadeIn('slow') // Show notification
			if (timer) {
				clearTimeout(timer); // Clear any previous timer
			}
			timer = setTimeout(function() { // Start timer to expire in 2 seconds
					$('#notify').fadeOut('slow'); // Hide notification
				}, 2000);
		$('#stat').show();
		});
	});
	...
	function ()
	{
	   $('.users-wrapper').load('users_widget.php .users-wrapper');
	}, 10000);
	</script>
